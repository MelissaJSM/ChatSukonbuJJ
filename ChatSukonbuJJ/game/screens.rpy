...
style melissa:
    ...
    background Image("gui/melissabox.png", xalign=0.5, yalign=1.0)
...
screen quick_menu() :
    ...
    frame at trans_qmenu : # 프레임이 trans_qmenu라는 트랜스폼을 적용하면서 나타남
        background # 배경은 투명하게 하고
        xysize (153, 66) # 편의상 버튼의 크기와 동일한 사이즈의 프레임으로
        align (0.5, 0.995) # 오른쪽 상단에 배치

        imagebutton : # 프레임 안에 이미지버튼이 있고
            idle 'gui/button_open.png' # 마우스가 올라가있지 않은 경우 button_open.png를 보여줌
            hover 'gui/button_open_p.png'
            action Show('qmenu')#, Hide('quick_menu') # 클릭하면 자기 자신 스크린을 숨기고 동시에 열린퀵메뉴(qmenu)를 보여줌

# 열린 퀵메뉴
screen qmenu() :

    # 3초 뒤에 닫힘과 동시에 기존의 스크린을 불러옴.
    timer 3 action Hide('qmenu')# Show('quick_menu') 

    frame at trans_qmenu : # 프레임이 trans_qmenu라는 트랜스폼을 적용하면서 나타남
        background # 배경은 투명
        xysize (1596, 167) # 프레임의 크기는 버튼 4개 + 간격을 고려해 500으로 잡음
        align (0.5, 0.995) # 우측 상단에 배치는 동일
        
        hbox : # 가로로 버튼들이 배열되어있음
            #spacing 226 # 간격은 20씩
            
            

            imagebutton : # 음성 토글 온 버튼(뿌부키)
                idle 'gui/button_voiceon.png'
                hover 'gui/button_voiceon_p.png'
                
                action Jump('voice_toggle_on')
            
            imagebutton : # 음성 토글 오프 버튼(미오)
                idle 'gui/button_voiceoff.png'
                hover 'gui/button_voiceoff_p.png'
                action Jump('voice_on')

            imagebutton : # 자동토글 버튼(페코)
                idle 'gui/button_toggle.png'
                hover 'gui/button_toggle_p.png'
                action Preference("auto-forward", "enable")

            imagebutton : # 종료 버튼(마린)
                idle 'gui/button_exit.png' # gui 폴더 아래의 png파일을 사용
                hover 'gui/button_exit_p.png'
                #action Hide('qmenu')#, Show('quick_menu') # 누르면 열린퀵메뉴를 닫고 기존의 접힌 버튼을 불러옴
                action Quit(confirm=True)

            imagebutton : # 닫기 버튼(미코)
                idle 'gui/button_fold.png'
                hover 'gui/button_fold_p.png'
                action Hide('qmenu')
            
            imagebutton : # 대화록 버튼(사메)
                idle 'gui/button_log.png'
                hover 'gui/button_log_p.png'
                action ShowMenu('history')

            imagebutton : # 설정 버튼(시간여행자의 은시계)
                idle 'gui/button_setting.png'
                hover 'gui/button_setting_p.png'
...
# 프레임에 적용되는 퀵메뉴의 움직임
transform trans_qmenu :
    
    # 나타날 때는
    on show :
        yoffset 1000 # 우측 바깥에 있다가
        pause 0.3 # 0.3초 뒤
        easein_expo 0.3 yoffset 0 # 0.3초동안 감속하면서 제자리로 등장

    # 사라질 때는
    on hide :
        yoffset 0 # 제자리에 있다가
        pause 0.3 # 0.3초 뒤에
        easeout 0.3 yoffset 1000 # 0.3초동안 가속하면서 우측 바깥으로 사라짐
...
screen main_menu():
    ...
    on "show" action Play("movie", "gui/movies/sukonbu_menu.webm", loop=True)
    on "hide" action Stop("movie")
    on "replace" action Play("movie", "gui/movies/sukonbu_menu.webm", loop=True)
...
screen about():
    ...
    use game_menu(_("버전정보"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("해당 시스템은 시라카미 후부키가 무료로 배포해준 스콘부짱의 live2D 및 ChatGPT를 사용하여 만들어졌습니다.\n\n해당 프로그램은 체험용 버전이며 gpt-4 1106 prev 엔진을 사용하였습니다.\n\n스콘부짱의 인격 수정이나 gpt 버전 변경이 필요하시면 깃허브의 링크를 참고하시기 바랍니다.\n\n프로그램 사용에 있어서 버그가 발생하였으면 다음을 통하여 문의 부탁드립니다.\n\n디시인사이드 멜리사J의 갤로그 : https://gallog.dcinside.com/jsm930602\n깃허브 멜리사J : https://github.com/MelissaJSM")
...
style confirm_frame:
    background Frame([ "gui/confirm_frame.png", "gui/system_alart.png"], tile=gui.frame_tile)
